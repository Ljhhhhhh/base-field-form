...
import { RuleObject, StoreValue, ValidateMessages, FieldError } from '../interface';
// import { defaultValidateMessages } from './messages';

const AsyncValidator: any = RawAsyncValidator;

async function validateRule(name: string, value: StoreValue, rule: RuleObject): Promise<string[]> {
...
export function validateRules(name: string, value: StoreValue, rules: RuleObject[]) {
  // let summaryPromise: Promise<string[]>;

  // >>>>> Validate by parallel
  const rulePromises = rules.map((rule) => validateRule(name, value, rule));

  const summaryPromise: Promise<string[]> = finishOnAllFailed(rulePromises).then(
    (errors: string[]): string[] | Promise<string[]> => {
      if (!errors.length) {
        return [];
      }
